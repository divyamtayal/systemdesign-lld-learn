...
import movingBookingDemo.entities.*;
import movingBookingDemo.enums.SeatStatus;
import movingBookingDemo.enums.SeatType;
import movingBookingDemo.strategy.payment.CreditCardPaymentStrategy;
import movingBookingDemo.strategy.pricing.WeekdaysPricingStrategy;
...
public class MovieBookingDemo {
    public static void main(String[] args) {
        ...
        // Add Cinemas
        Cinema amcNYC = service.addCinema("cinema1", "AMC Times Square", nyc.getCityId(), List.of(screen1));

        // Add Shows
        Show matrixShow = service.addShow("show1", matrix, screen1, LocalDateTime.now().plusHours(2),
                new WeekdaysPricingStrategy());
        ...
        List<Seat> availableSeats = selectedShow.getScreen().getCapacity().stream()
        ...
        System.out.printf("Available seats for '%s' at %s: %s%n",
                ...
                availableSeats.stream().map(Seat::getSeatId).collect(Collectors.toList()));

        // 3. Select seats
        List<Seat> desiredSeats = List.of(availableSeats.get(2), availableSeats.get(3));
        System.out
                .println("Alice selects seats: " + desiredSeats.stream().map(Seat::getSeatId).collect(Collectors.toList()));
        ...
        if (bookingOpt.isPresent()) {
            ...
            System.out.println("Seats: " + booking.getSeats().stream().map(Seat::getSeatId).collect(Collectors.toList()));
        ...
        desiredSeats.forEach(seat -> System.out.printf("Seat %s status: %s%n", seat.getSeatId(), seat.getSeatStatus()));
