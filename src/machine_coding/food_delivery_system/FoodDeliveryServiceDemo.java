package machine_coding.food_delivery_system;

import java.util.*;
import machine_coding.food_delivery_system.entities.*;
import machine_coding.food_delivery_system.enums.OrderStatus;
import machine_coding.food_delivery_system.strategy.assignment.NearestAvailableAgentStrategy;
import machine_coding.food_delivery_system.strategy.search.RestaurantSearchStrategy;
import machine_coding.food_delivery_system.strategy.search.*;
...
public class FoodDeliveryServiceDemo {
        public static void main(String[] args) {
                ...
                service.registerDeliveryAgent("Bob", "321-4567-880",
                                new Address("1 B", "Springfield", "12345", 40.71, -74.00));
                ...
                List<RestaurantSearchStrategy> proximitySearch = List
                                .of(new SearchByProximityStrategy(aliceAddress, 0.01));
                ...
                nearbyRestaurants.forEach(r -> System.out.printf("  - %s (Distance: %.4f)\n", r.getName(),
                                aliceAddress.distanceTo(r.getAddress())));
                ...
                pizzaMenu.getItems().values()
                                .forEach(item -> System.out.printf("  - %s: $%.2f\n", item.getName(), item.getPrice()));
                ...
                if (!pizzaRestaurants.isEmpty()) {
                        ...
                        var order = service.placeOrder(alice.getId(), chosenRestaurant.getId(),
                                        List.of(new OrderItem(chosenItem, 1)));
